...
public interface ArticleEntityRepository extends JpaRepository<ArticleEntity, Integer> {
    ...
    Page<ArticleEntity> findArticleEntitiesByDeletedAndSoldAndBuyerPersonUsername(Boolean deleted, Boolean sold, String username, Pageable pageable);
    ...
    @Query("SELECT DISTINCT u FROM ArticleEntity u LEFT JOIN u.attributes attr WHERE u.deleted=:deleted and (:isNew is null or u.isNew = :isNew) and u.sold=:sold and (:category is null or u.articleType.name LIKE :category) and (:locationId is null or u.user.location.id=:locationId) and (:search is null or u.title LIKE %:search%) and (:priceFrom is null or u.price >= :priceFrom) and (:priceTo is null or u.price <= :priceTo) and ((:namesSize = 0) or attr.name IN :names) and ((:valuesSize = 0) or attr.value IN :values) group by u.id having ((:namesSize=0) or count(u) = :namesSize)")
    Page<ArticleEntity> findArticleEntitiesByTypeIdWithQuery(Boolean deleted, Boolean sold, Integer locationId, String search, Boolean isNew, BigDecimal priceFrom, BigDecimal priceTo, String category, @Param("names") List<String> names, @Param("values") List<String> values, int namesSize, int valuesSize, Pageable pageable);
}
