...
public class WebSecurityConfig{
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        http = http.cors().and().csrf().disable().authorizeHttpRequests()
                ...
                .requestMatchers(HttpMethod.GET, "/messages/user", "/articles/bought/user/{name}").authenticated()
                .requestMatchers("/articles/search", "/auth/**", "/articles/all", "/articles/sold/**", "/articles/active/**", "/articles/info/**", "/articles/type/**", "/article-types/**", "/article-types/{name}/options", "/users/**", "/attribute/**", "/locations", "/logs")
                .permitAll()
                .anyRequest()
                .authenticated()
                .and()
